...
public class CharacterController2D : MonoBehaviour
{
	...
	void Awake()
	{
        ...
        void Update()
        {
            if (!isGrounded)
            {
                this.GetComponent<Animator>().SetBool("Ground", false);
                this.GetComponent<Animator>().SetBool("Slide", false);
            }
            else
            {
                this.GetComponent<Animator>().SetBool("Ground", true);
                if (Input.GetKey(KeyCode.Z))
                {
                    this.GetComponent<Animator>().SetBool("Slide", true);
                    this.GetComponent<BoxCollider2D>().offset = new Vector2((float)-0.1373292, (float)-1.506965);
                    this.GetComponent<BoxCollider2D>().size = new Vector2((float)4.501842, (float)4.09393);
                }
                else
                {
                    this.GetComponent<Animator>().SetBool("Slide", false);
                    this.GetComponent<BoxCollider2D>().offset = new Vector2((float)-0.1373292, (float)0);
                    this.GetComponent<BoxCollider2D>().size = new Vector2((float)4.501842, (float)7.11);
                }
            }
